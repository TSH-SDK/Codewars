...
# "Same" means, here, that the elements in b are the elements in
...
# (except in C++, COBOL, Crystal, D, Dart, Elixir, Fortran,
# F#, Haskell, Nim, OCaml, Pascal, Perl, PowerShell, Prolog,
...
def comp(ar1, ar2):
    tr_list = []
    if len(tr_list) == 0:
        if isinstance(ar1, list) and isinstance(ar2, list):
            if len(ar1) == len(ar2):
                for i in ar1:
                    if ar1.count(i) == ar2.count(i ** 2):
                        if i ** 2 in ar2:
                            tr_list.append(True)
                        else:
